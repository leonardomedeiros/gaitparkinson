...
\title{Gait Analysis to Track Parkinson's Disease Evolution Using Reproducible Research Practices}
...
Yang et al.~\cite{igait2012} present an accelerometer-based gait analysis system. They provide a graphical interface to display and analyze the gait acceleration data recorded by an accelerometer attached to the lower back of subjects. This system calculates a total of four gait features: spatio-temporal, frequency domain, regularity and symmetry. However, they do not calculate walking velocity; it is required that the users input this information. %An important feature is allowing users to select regions of gait events for a given signal, displaying the results on the screen and saving them for further statistical analysis. So, looking for this related work with could provide an assessment mechanism of this training approach with the evaluation of the PD's patient gait characteristic.

Dillman et al.~\cite{dillmann2014} used Principal Component Analysis (PCA) to analyze movements of the upper and lower extremities during treadmill walking with healthy subjects and two groups of PD's patients. This study considered 35 control subjects and 36 PD patients. They aimed to analyze the value of PCA to characterize dynamic changes of gait, focusing on the gait velocity and the severity of the disease.  %In our work, we used the PCA values to track the gait characteristics of PD symptoms using the projection of the PCA values into the \textit{eigenspace}. With this projection, we classified data based on the euclidean distance of the training data and compared with the validation test data.
...
%\footnote{\textbf{Our Reproducible Research website will be available after the notification results. Because, the EasyChair submission system only accepts PDF files.}}
...
%    Abstract.
%    Full reference of your paper, with current publication status, and a PDF of your paper. This allows others to cite it correctly!
%    The code to reproduce all the results, images and tables. Make sure all the code is well documented, and that there is a readme file explaining how to execute it.
%    The data (images, measurements, etc) to reproduce all the results, images and tables. Add a readme file explaining what the data represent.
...
%Donoho Harmonic
%We publish not just computational results but also the complete software environment and data that generated those results.
...
%, which was used to perform the gait analysis of this work through of the signal processing techniques we afford: identify, quantify the gait characteristics~\cite{neumankensiology}. 


%The study of the human gait includes of the forces and momentum that result in the movement of body segments in a human gait includes the measurement of the Vertical Ground Reaction Force (VGRF) \cite{neumankensiology} and then gait analysis~\cite{gaitusingsensorsreview2012}. The VGRF of our database was captured by force sensors attached into the shoes. 
...
% * <mperkusich@gmail.com> 2015-09-14T17:53:01.188Z:
%
%  Quais estudos????
%


%PCA is largely used for face recognition. However, by combining gait analysis and PCA, it is possible to visualize the difference of the movement characteristics between PD patients and healthy subjects. This method is an adaptation of the PCA largely used for face recognition. This method can be used to distinguish the gait characteristics of PD patients~\cite{dillmann2014}. 
...
% * <mperkusich@gmail.com> 2015-09-14T18:10:13.427Z:
%
%  o que vc quer dizer com isso? (em pt-br)
% Na Figura ~\ref{fig:diferencemean} temos os vetores médios escalonados em 80 medições que foram utilizados para calcular o PCA deste trabalho. Como podemos perceber na Figura XX existem diferenças significativas entre o grupo de controle e os indíviduos parkinsonianos. O PCA será utilizado justamente para identificar essas diferenças e permitir a projeção de uma eventual melhora ou piora do quadro clínico.
% ^ <mperkusich@gmail.com> 2015-09-14T18:44:16.399Z.
...
% * <mperkusich@gmail.com> 2015-09-16T02:39:06.282Z:
%
%  Ajeite a figura colocando os nomes dos componentes: GAIT ACQUISITION COMPONENT e SIGNAL PROCESSING COMPONENT
%
...
%Signals acquired by motion sensors have a lot of noise. Therefore, identifying the motion cycles is challenging, because they have a position which starts the cycle movement as in Figure 3.2 and the existing noise can cross this line and consequen fearing generate fake IDs. Methods that make low-pass filter can use be applied to smooth the curve and reduce the occurrence of noise, but this implies a change in time of the signal which directly impacts the calculation of the features movement and as a result the accuracy of the final result.
...
%In the literature, we found Gait Analysis workos applied to PD's patients evaluation. 

%This is a standard that establishes the requirements for the safe integration of medical devices and other equipments in a edical system. Its purpose is aiding medical systems to have greater resistance to errors, improving patient safety and effectiveness of their treatment.

%Using the same Physionet Parkinson Disease Database~\cite{physionet}, Rodrigo et al.~\cite{rodrigo2012} used Kohonen map for clustering of locomotion of the gait characteristics comparing with PD's patients and control group subjects. The extracted features was based on Mean coefficient of variation (CV) and Mean sum of VGRF during consecutive stance phases to describe the inter-subject variability. From the analysis of these results the Kohonen map was able to recognize: TP of 72.09$\%$ and TN of 70.21$\%$.

%Yang et al.~\cite{igait2012} developed a accelerometer based gait analysis system. They provide a graphical interface to display and analyze the gait acceleration data recorded by an accelerometer attached to the lower back of subjects. This system, calculates a total of four gait features: spatio-temporal, frequency domain, regularity and simmetry. But, with the accelerometer data they couldn't calculate the walking velocities. So, this information must by inputted by the user. An important feature is allows the user select the regions of gait events for a given signal and display the result on the screen and save the results for further statistical analysis.  So, looking for this related work with could provide an assessment mechanism of this training approach with the evaluation of the PD's patient gait characteristic.

%Dillman et al.~\cite{dillmann2014} used PCA to to analyze movements of the upper and lower extremities during treadmill walking in healthy subjects and two groups of PD's patients. With a study of  35 control subjects and 36 patients with PD patients. They aimed to analyze the value of PCA to characterize dynamic changes of gait. Focusing on the gait velocity and severity of the disease. In our work, we intent to use the PCA values to track the gait characteristics of PD symptoms using the projection of the PCA values into the \textit{eigenspace}. With this projection we did a data classification based on the euclidean distance of the training data and compared with the validation test data.

%Mazilu et al.~\cite{mazilu2015}, investigated the acceptance of a wearable gait assist for PD's patients at their home. They developed a gait training system that detected FoG events. So, with the FoG detection, they used a rhythmic auditory signal that serves as gait stimulation to support the user to alleviate the FoG symptom and resume the gait movement. So, looking for this related work with could provide an assessment mechanism of this training approach with the evaluation of the PD's patient gait characteristic.
...
%So, as we are using only free software and a open data set - 5: The results can be easily reproduced by an independent researcher with at most 15 min of user effort, requiring only standard, freely available tools (C compiler, etc.).  According the reproductbility degrees of this research, we intent ~\cite{vandewalle2009}





%as you can see the results in the (Fig. 1).
%PD motor symptoms are usually monitored through motion sensors or video cameras ~\cite{robotgait2014,patel_monitoring_2009,mazilu2015}. Those devices are difficult to be seamlessly integrated in the user routine. 
%PhysioBank databases are made available under the ODC Public Domain Dedication and License v1.0. 
...
% * <mperkusich@gmail.com> 2015-09-14T18:56:07.299Z:
%
%  O que vc quis dizer com isso? (pt-br)
%RESPOSTA assim está melhor ? (alterei o texto)
% ^ <mperkusich@gmail.com> 2015-09-14T19:26:22.495Z:
%
%  "Through signal processing technique for VGRF signal data acquired by foot sensors"... traduz isso pra pt-br
%
%
% ^ <mperkusich@gmail.com> 2015-09-14T19:30:42.922Z.
% * <mperkusich@gmail.com> 2015-09-14T18:55:30.545Z:
%
%  esses dados sao de qual grupo de subjects?
%RESPOSTA: 
% dos dois grupos (PD e grupo de controle)
% ^ <mperkusich@gmail.com> 2015-09-14T19:26:27.915Z.
% * <mperkusich@gmail.com> 2015-09-14T18:54:08.657Z:
%
%  a escolha foi aleatoria?
% Na verdade não consegui processar alguns arquivos uns pq o ciclo era muito rápido ou cheio de ruídos e outros pq os pacientes com parkinson simplesmente não tinham fase de BaLANÇO ( ou seja eles viviam com os pés no cchão, logo não conseguia identificar os ciclos de movimento).
%Então consegui um número de 50 arquivos de parkinson e refleti o mesmo número para o grupo de controle.
%
% ^ <mperkusich@gmail.com> 2015-09-14T19:26:31.622Z.
...
%\subsection{PCA For Gait Analysis}
...
% * <mperkusich@gmail.com> 2015-09-15T01:41:49.167Z:
%
%  Aqui vc ta referenciando a Figura 5, certo?
        %LMM: Sim.
% ^ <mperkusich@gmail.com> 2015-09-15T01:45:25.344Z.

%As example you we can see the trajectories of two persons, one with symptoms and one without. So, the Parkinson subject reduced her symptoms along the time.
...
%So, in our analyses a patient's personal gait pattern is represented as a point into the \textit{eigenspace}. Changes in the patient gait pattern will result in a moving of this point. In case the point moves towards the direction of the subjects with PD, the system detects an increase of PD symptoms for the patient.


%An individual identified as a carrier on the basis of PD is represented as a point in this \textit{eigenspace}. 
...
% Cross-validation consists in partitioning the data set into subsets that are mutually exclusive and with the same size. It uses some of these subsets as training data to find values which allow prediction of the remaining subsets, used to validate the predictive model~\cite{datamining2005}.
% * <mperkusich@gmail.com> 2015-09-15T02:07:01.812Z:
%  
%  O que vc quiser dizer com isso? pt-br...
%  LMM: Pode reduzir bastante esse texto. Ou até remove acho que está redundante.
%
...
% * <mperkusich@gmail.com> 2015-09-15T02:09:39.467Z:
%
%  Vc já disse que usou o 10 k-fold... PQ ta dizendo de novo a mesma coisa? Como é que lá em cima vc diz que usou a técnica e só aqui vc a descreve?
%
...
% \begin{table}[!htbp]
% \caption{Confusion Matrix Of PCA Classification 10 K-Fold Cross Validation}
% \label{table:resultadomatrizconfusaosvm}
% \centering
% \begin{tabular}{ll|c|c|}
% \cline{3-4}
%                                                                                                                 & \multicolumn{1}{c|}{}  & \multicolumn{2}{c|}{\textit{\textbf{Predicted Class}}} \\ \cline{3-4}
%                                                                                                                 &                        & \textbf{Parkinson}       & \textbf{Control Group}      \\ \hline
% \multicolumn{1}{|c|}{\multirow{\textit{\textbf{\begin{tabular}[c]{@{}c@{}}Actual\\ Class\end{tabular}}}}} & \textbf{Parkinson}     & 429       

%                 & 71                           \\ \cline{2-4}
% \multicolumn{1}{|c|}{}                                                                                          & \textbf{Control Group} & 114                        & 386          
...
%As we showed in this work, the gait analysis can vary from person to person, and besides c. However, the present approach allows us to identify patterns characteristics of PD's gait patterns between PD's subjects and Control Group. So, we demonstrate this monitoring gait approach could be used for foot sensor data to track the PD's symptoms.
